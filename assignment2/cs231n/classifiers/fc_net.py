...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    w2 = self.params['W2']
    b2 = self.params['b2']
    layer1_out, cache1 = affine_relu_forward(X, w1, b1)
    layer2_out, cache2 = affine_forward(layer1_out, w2, b2)
    scores = layer2_out
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
    ...
    dlayer1_out, dw2, db2 = affine_backward(dscores, cache2)
    dw2 += self.reg * w2
    dX, dw1, db1 = affine_relu_backward(dlayer1_out, cache1)
    dw1 += self.reg * w1
    grads['W1'] = dw1
    grads['b1'] = db1
    grads['W2'] = dw2
...
class FullyConnectedNet(object):
  ...
  def loss(self, X, y=None):
    ...
    for i in range(self.num_layers - 1):
      ...
      outs.append(out)
      caches.append(cache)
    #forward 1 affine layer
    last_layer = self.num_layers - 1
    W = self.params['W' + str_idx(last_layer)]
    b = self.params['b' + str_idx(last_layer)]
    scores, cache_final = affine_forward(inp, W, b)
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
    ...
    for i in reversed(range(self.num_layers - 1)):
      ...
      if self.use_batchnorm:
        dout, dw, db, dgamma, dbeta= \
          affine_batch_relu_backward(dout, cache)
